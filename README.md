...
The app's specs set the **minimum shares**: each pool's floor is the spec divided by the server's spec (via `GPU_VRAM_GB`/`GPU_TFLOPS` — note `GPU_VRAM_GB` is the tenant-sellable budget, not the card: the gpu flavor sets 45 of the H200's 141 GB because the co-hosted vLLM platform model holds the rest; compute is 989 TFLOPS; node = 64 GB / ~1000 GFLOPS via `NODE_RAM_GB`/`NODE_GFLOPS`; CPU compute is denominated in GFLOPS because a whole node is only ~1/1000 of a card), taking the **larger** of the memory and compute axes, rounded up to the whole percent. A GPU app's CPU minimum also lifts its GPU minimum, because of the invariant: **`gpuShare >= cpuShare` whenever `gpuShare > 0`** (a GPU app's CPU slice rides on the same node as its card). Runners enforce the minimums at deploy and claim time; the site's deploy page floors its two dials at them.
