//! vault: the web UI for Enclave's wallet-gated encrypted volumes.
//!
//! A tiny wasi:http component that serves a self-contained browser app. All
//! of the cryptography (scripts/enclave-vault.mjs via @noble), the chain access
...
//!   GET /config.json  - embedded defaults overlaid with ENCLAVE_CONFIG (the
//!                       deployment's CID-verified configCid JSON): contract
//!                       addresses, RPC, chain id, attestation repo.
//!   GET /ping         - liveness.
//!
//! What the app does (the browser side of scripts/enclave-vault-client.mjs):
//! connect wallet -> sign DERIVE_MESSAGE -> derive X25519 -> register the
//! pubkey on EnclaveVolumeAccess -> unseal own sealedVEK -> resolve the
//! deployment's runner enclave from chain state (EnclaveDeployments.get(id).runner
//! = keccak256 of the EnclaveRegistry endpoint) -> verify THAT enclave's
...
/// deployment through ENCLAVE_CONFIG (shallow merge, unknown keys pass through).
