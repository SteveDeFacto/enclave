...
scp relay.js tcp6-relay.js udp-relay.js egress-relay.js net-guard.mjs package.json nan-relay:/opt/nan-relay/
...
# (ENCLAVE_URL + EGRESS_RELAY_TOKEN + EGRESS_PREFIX=<same /64>). Until then its
# restart is a no-op failure; enable it explicitly when the operator adds the env.
